...
build:
	...
	@ docker-compose -f ${DEV_COMPOSE_FILE} exec backend ${DEV_FOLDER}/start_app.sh

frontend-tests:
	@ echo "Converge frontend tests running..."
...
migrate-initial:
	...
	@ docker-compose -f ${DEV_COMPOSE_FILE} exec backend ${DEV_FOLDER}/start_initial_migrations.sh "$(message)"

migrate:
	@ echo 'Running migrations...'
	@ docker-compose -f ${DEV_COMPOSE_FILE} exec backend ${DEV_FOLDER}/start_migrations.sh "$(message)"
...
test:
	@ echo 'start tests...'
	@ docker-compose -f ${DEV_COMPOSE_FILE} exec backend ${DEV_FOLDER}/start_tests.sh "$(test)"
