...
class Preview extends Component {
   ...
   * @returns {function}
   */
  makeLevelActiveClick = (id, type) => () => this.setState({
   ...
   * @returns {function}
   */
  makeLevelActiveHover = (id, type) => () => this.setState({
  ...
  /**
   * check if there are more hightlighted buttons on the next or previous pagination
   *
   * @param {integer} index
   * @param {integer} level
   * @param {object} data
  ...
  checkMoreHighlighted = (index, level, data) => {
    ...
    for (let i = 0; i < data.length; i += 1) {
      if (activeLevel === data[i].parentId || activeLevel === data[i].id
  ...
  renderDeleteIcon = (counter, values, data, child) => {
    ...
    return removeable && <button className="remove-level" onClick={this.props.removeLevel(child)}>x</button>;
  };
        ...
        <span>
          <div className="preview-levels" id={level}>
            <div className="levels-grp">
              {
                // identify level
                values.nameObj.map((child, index) => (
                  ...
                  <div className="preview-btn-container" key={child.id}>
                    ...
                    {(index >= 1 && activeLevelPagination === index
                      && values.nameObj.length > 4)
                      ?
                        <button onClick={e => this.slide(e, index, level, 'previous')} className="arrow__left">
                          <img src={chevronIcon} className={paginationIconClass.previous} alt="chevron icon" />
                        </button>
                      : ''
                    }
                    <button
                      className={
                        (activeLevel === child.parentId || activeLevel === child.id
                    ...
                    { (values.nameObj.length > 4 && index < values.nameObj.length - 1
                      && index === activeLevelPagination + 3)
                      || (values.nameObj.length > 4 && index === 3)
                    ?
                      <button className="arrow__right" onClick={e => this.slide(e, index, level, 'next')}>
                        <img src={chevronIcon} className={paginationIconClass.next} alt="chevron icon" />
                      </button>
                    : ''
                    }
                  </div>
